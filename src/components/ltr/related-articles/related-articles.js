...
import { getLatestArticles, getStrapiMedia, formatDate } from "@/lib/strapi";
...
const RelatedArticles = ({ categorySlug }) => {
  ...
  // Split into groups of 3 for carousel-like display
  const groups = [];
            ...
            <div key={article.id} className="col-xs-6 col-sm-4 col-md-4 col-padding">
              <div className="grid-item">
                <div className="grid-item-img">
                  <Link href={`/${language}/article/${data.slug}`}>
                    <img
                      src={imageUrl}
                      className="img-fluid"
                      alt={data.title}
                ...
                <h5>
                  <Link href={`/${language}/article/${data.slug}`} className="title">
